# src/mf_scoring.py
# ============================================================
# MF SCORING (Master / Pro)
# ------------------------------------------------------------
# Outputs (one row per MF player):
#   - mf_post_rating_0100          : OFFICIAL (within DM / CM / AM)
#   - mf_role_rating_0100          : within role (e.g., DM_CONTROLLER)
#   - mf_role_rating_shrunk_0100   : role rating shrunk toward post rating
#
# Weights:
#   - Within each POST (DM / CM / AM), run ANOVA across ROLES for each feature
#   - Use omega-squared (ω²) effect size as "importance"
#   - Normalize ω² into weights
#
# Robust rating:
#   - Use percentile-rank 0-100 (with winsorization) instead of raw min-max
#
# Assumptions about your pipeline:
#   - df_mf (from assign_mf_positions): Player, Squad, mf_post
#   - df_dm_roles: Player, Squad, mf_role (DM roles)
#   - df_am_roles: Player, Squad, mf_role (AM roles)
#   - df_cm_roles: Player, Squad, cm_role (CM roles)
...
# -----------------------------
DM_ROLES = ["DM_CONTROLLER", "DM_DESTROYER", "DM_DEEP_CM"]
CM_ROLES = ["CM_PROGRESSOR", "CM_BOX_TO_BOX"]
AM_ROLES = ["AM_ORGANIZER", "AM_CLASSIC_10"]


# -----------------------------
# Feature space (rich but sane)
# IMPORTANT:
# - We'll compute weights per POST (DM/CM/AM), not across posts.
# - Features not discriminant get ~0 ω² automatically.
# -----------------------------
COMMON_MF_FEATURES = [
    # Touch / zones
    ...
    "Pass_Switch", "Pass_TB", "Pass_Cross",

    # Progression / threat via pass
    "PrgP", "PrgDist",
    "FinalThird_Pass", "PPA", "CrsPA",
    "KP", "xA", "xAG", "A-xAG",

    # Creation chains
    "SCA90", "GCA90", "SCA", "GCA",

    # Progression via carry / receiving
    "PrgC", "Carry_PrgDist", "Carry_FinalThird", "Carry_PenArea",
    "Rec", "PrgR",

    # Take-ons (press resistance / AM/CM)
...
# Post-specific extras (optional, small)
DM_EXTRA = ["Aerials_Won", "Aerials_Won%"]
CM_EXTRA = []  # keep light
AM_EXTRA = ["Sh/90", "SoT/90", "xG", "npxG"]  # only for AM post weights


@dataclass(frozen=True)
class PostCfg:
    name: str              # "DM", "CM", "AM"
    mf_post_value: str     # "DM" / "CM" / "AM"
    role_col: str          # where the role label is stored in merged table
    valid_roles: List[str]
    extra_features: List[str]


POSTS = [
    PostCfg("DM", "DM", "mf_role", DM_ROLES, DM_EXTRA),
    PostCfg("CM", "CM", "cm_role", CM_ROLES, CM_EXTRA),
    PostCfg("AM", "AM", "mf_role", AM_ROLES, AM_EXTRA),
]
...
def _rating_0100_from_rank(s: pd.Series, clip: float = 0.02) -> pd.Series:
    ...
    clip=0.02 means 2% winsorization on both ends.
    """
    s = s.astype(float)
    if s.notna().sum() < 3:
...
# ============================================================
# Merge table MF (posts + roles + stats)
# ============================================================
...
def build_mf_merged(
    ...
    """
    Build one MF table with:
      Player, Squad, mf_post, mf_role (DM/AM), cm_role (CM),
      Min_num + all stats from df_outfield.
    """
    # DM+AM roles share 'mf_role'
...
# ============================================================
# Weights per POST (DM, CM, AM) based on ANOVA across roles
# ============================================================
...
def compute_post_weights(
    ...
    sub = df[df["mf_post"] == post.mf_post_value].copy()
    ...
    feats = _dedupe(COMMON_MF_FEATURES + post.extra_features)
    ...
    for f in usable:
        ...
        rows.append({"mf_post": post.name, "feature": f, "omega2": w2})
...
def score_midfielders(
    ...
    """
    Returns:
      weights_df   : weights per post (DM/CM/AM)
      df_scored    : MF table with post + role ratings (plus shrunk role)
    """

    df_all = build_mf_merged(df_outfield, df_mf, df_dm_roles, df_cm_roles, df_am_roles)
    ...
    df_scored["mf_post_score_raw"] = np.nan
    df_scored["mf_post_rating_0100"] = np.nan

    df_scored["mf_role_score_raw"] = np.nan
    df_scored["mf_role_rating_0100"] = np.nan
    df_scored["mf_role_rating_shrunk_0100"] = np.nan

    df_scored["n_post"] = np.nan
    df_scored["n_role"] = np.nan
    df_scored["role_sample_flag"] = pd.Series(index=df_scored.index, dtype="string")

    # score per post (DM/CM/AM)
    for post in POSTS:
        post_mask = (df_scored["mf_post"] == post.mf_post_value) & (df_scored[post.role_col].isin(post.valid_roles))
        post_df = df_scored.loc[post_mask].copy()

        w_post = weights_df[weights_df["mf_post"] == post.name].copy()
        feats = w_post["feature"].tolist()

        # POST scoring (within post)
        post_raw = score_weighted_z(post_df, w_post, feats)
        df_scored.loc[post_df.index, "mf_post_score_raw"] = post_raw
        df_scored.loc[post_df.index, "mf_post_rating_0100"] = _rating_0100_from_rank(post_raw)
        ...
        for r in post.valid_roles:
            ...
            df_scored.loc[ridx, "mf_role_score_raw"] = role_raw
            df_scored.loc[ridx, "mf_role_rating_0100"] = _rating_0100_from_rank(role_raw)
    ...
    df_scored["mf_role_rating_shrunk_0100"] = _shrink_rating(
        df_scored["mf_role_rating_0100"],
        df_scored["mf_post_rating_0100"],
        df_scored["n_role"],
        k=shrink_k,
    )
